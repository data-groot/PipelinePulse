...
- Frontend: Next.js 16 (App Router), TypeScript, Tailwind v4, shadcn/ui on **Base UI** (`render` prop, NOT Radix `asChild`), TanStack Query v5, Recharts. Deployed on Google Cloud Run (standalone output + Dockerfile).
- Backend: FastAPI, Python 3.12, SQLAlchemy 2 async, Pydantic v2. Deployed on Google Cloud Run (scale-to-zero).
- DB: PostgreSQL 16 — local via docker-compose, prod on Cloud SQL (unix socket via `--add-cloudsql-instances`). Tables created by `create_all` on startup (no Alembic). See DEPLOYMENT.md.
...
- Auth is httpOnly-cookie JWT ONLY. No localStorage tokens, no Bearer headers. The frontend proxies `/api/*` and `/auth/*` to the backend in `proxy.ts` (Next 16 middleware; `BACKEND_URL` env read at request time, NOT in next.config rewrites which freeze at build), so requests are same-origin — do not add CORS workarounds.
